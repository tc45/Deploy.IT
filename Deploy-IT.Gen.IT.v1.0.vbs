...
Dim g_ConnectionType, g_ConnectionPort
...
g_DeviceType = "<<DEVICE_TYPE>>" 											' Set to specify device type (Router|Switch|ASA)
g_DeviceHostname = "<<SWITCH_HOSTNAME>>"									' Set to specify device hostname (final hostname)
g_DeviceIOS	= "<<DEVICE_IOS>>"												' Set to specify IOS image file
g_DeviceIOSInstall = "<<SWITCH_SOFTWARE_INSTALL>>"							' Set to specify IOS image deployment type (Standard|Archive|Software Install)
g_DeviceProvisioningIF = "<<DEVICE_PROVISION_IF>>"							' Set to specify Provisioning interface (Gig0/1, Gig1/0/1/, Vlan1)
g_DeviceFlash = "<<DEVICE_FLASH_TYPE>>"										' Set to specify flash device name (e.g. flash, bootdisk, disk0, etc) (will set to flash if not specified)
g_SCP_Host = "139.46.114.21"												' Set to SCP Server hostname/IP for file transfer of IOS/config file
g_SCP_Username = "devsetup"													' SCP Username
g_SCP_Password = "S72ctAd_sWe96pe"											' SCP Password

g_ConnectionType = ""														' use if script will connect to optional terminal server.  Type MRV|Cisco|WTI
g_ConnectionPort = ""														' Port will specify post-connection port to reverse telnet to.
...
Sub Main
	...
	If lcase(sConnectionType) = "cisco_term" Then 
		Call Get_ConnectConsolePort (sConnectionType, sPortNumber, sHostname)
	End If
